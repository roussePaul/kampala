...
The GUI consists of several plugins in rqt, a framework for GUI development in ROS based on the application framework Qt. It is started with the command \texttt{roslaunch scenarios rqt.launch}. To launch the GUI properly, one needs to be in a catkin workspace with the \texttt{kampala} directory cloned from GitHub in its src directory, as the path from the directory where you launch the GUI to the files in the \texttt{kampala} is hardcoded in several of the plugins. When launched, a window with the header \texttt{Default - rqt} should appear. If the GUI hasn't been used before, the window should be empty, but you can add plugins to it by clicking on the \texttt{Plugins} tab. A dropdown menu with different folders will appear and the plugins added specifically for the quadcopters will be in the Iris folder. The most important plugin is the \texttt{Start interface}, which is needed to start the quadcopter. Its use is described in the section \texttt{How to fly}. Another important plugin is the \texttt{Tabbed GUI plugin}. It contains four tabs with one function each. They should be fairly self-explanatory but otherwise one should refer to the documentation in the code behind them, which can be found in the \texttt{gui/src/gui} directory. The files with the name \texttt{rqt iris} define the \texttt{Start Interface}, the \texttt{RCDisplay} files define the \texttt{RC and battery display}, the \texttt{pointInput} files define the \texttt{Instruction input}, the \texttt{saver} files define the \texttt{Data recorder} and the \texttt{positionPlot} files define the \texttt{Plot windows} tab.   

\subsection{Advice on modifying the GUI}

The GUI has been created mostly by using the editor \texttt{QtCreator}. An installation guide for the editor can be found here:  \url{https://wiki.qt.io/Install_Qt_5_on_Ubuntu}. To modify a plugin's appearance with this editor, double-click on the corresponding ui-file. With the editor, you can drag and drop widgets like buttons and text fields from a menu to add them to the plugin. Double-clicking on a widget lets you rename it. The logic in the GUI plugins is not defined in the ui file but in a python file that integrates the ui file. It is also possible to add widgets programmatically by importing them in the python file and adding them there. Refer to the file \texttt{positionPlot.py} for an example of that. At the end of the \texttt{README} file in the \texttt{gui} directory, a link to a ROS tutorial about creating rqt plugins can be found. This tutorial also links to other tutorials about integrating ui-files into python files.

The most important piece of advice when writing the code for the rqt-plugins is the following: \texttt{Do not try to modify Qt objects from a thread other than the main thread}. Qt objects are not thread safe and if you try to reach them from another thread, the GUI plugin can randomly freeze, or even worse, the whole GUI can crash. Trying to modify Qt objects in \texttt{rospy.Subscriber} callbacks counts as this, since a new thread is created for the callback function. What you can do is emit a signal from the other thread and connect it to a function that does the required modifications of the Qt object, since this function will be called from the main thread. Do as little as possible in that function and don't call it too often, as tasks executed in the main thread slow the GUI down. The following link shows some examples of using signals in python after importing the module PySide: \url{https://wiki.qt.io/Signals_and_Slots_in_PySide}    
