...
class Identification:
	...
	def __init__(self,method="areas"):
		...
		self.r_param = {"amplitude":1.0, "oscillations":4, "hysteresys":0.1}

		# state if the identification process
		# states: "wait", "initialize" "in progress", "identify", "done"
		self.state = "wait"

	def get_command(self,time, input):
		if self.method == "areas":
			return self.step(time,input)
		if self.method == "areas":
			return self.step(time,input)
	...
	def relay(self,time,input):
		if self.state == "initialize":
			self.n_switch = 0
			self.current_U = self.r_params["amplitude"]
			self.initial_time = time 
			self.y = []
			self.time = []
			self.state = "in progress"
		...
		if self.state == "in progress":
			E = self.r_params["hysteresys"]
			D = self.r_params["amplitude"]
